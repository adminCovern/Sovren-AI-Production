/**
 * SOVREN AI - Zoho Webhook Handler API Route
 * 
 * Handles incoming Zoho webhooks for automated billing events.
...
function initializeBillingSystem() {
  if (!billingSystem) {
    ...
    console.log('🏦 Billing system initialized for Zoho webhooks');
  }
}
...
export default async function handler(
  ...
  try {
    ...
    if (!signature) {
      console.error('❌ Missing Zoho signature header');
      return res.status(400).json({
        error: 'Missing signature',
        message: 'X-Zoho-Signature header is required'
      });
    }
    ...
    // Process webhook
    const result = await webhookHandler.handleZohoWebhook(buf, signature);

    if (result.success) {
      console.log(`✅ Zoho webhook processed successfully: ${result.eventType} (${result.eventId})`);
      ...
      console.error(`❌ Zoho webhook processing failed: ${result.error}`);
    ...
    console.error('❌ Zoho webhook handler error:', error);
