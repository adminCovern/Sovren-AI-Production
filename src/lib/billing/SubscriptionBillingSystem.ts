...
// Zoho Payments removed - using Stripe only





// Subscription tiers configuration (Stripe only)
...
export class SubscriptionBillingSystem extends EventEmitter {
  ...
  constructor() {
    ...
    console.log('🏦 Subscription Billing System initialized (Stripe only)');
  }
  ...
  public async initializeUserBilling(user: BillingUser): Promise<{
    ...
    try {
      // Create Stripe customer (Stripe only - no fallback)
      ...
      this.gatewayManager.recordFailure('stripe', error as Error);
      console.error('Stripe customer creation failed:', error);

      return {
        success: false,
        requiresPayment: false,
        error: 'Stripe payment gateway unavailable'
      };
    }
  }
  ...
  public async processInitialPayment(
    ...
    // Process payment through Stripe only
    try {
  ...
  // Zoho payment processing removed - using Stripe only
